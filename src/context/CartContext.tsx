...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  console.log('CartProvider: State initialized');
  
  try {
    console.log('CartProvider: Getting auth context...');
    const { user } = useAuth();
    ...
    const clearCart = async () => {
      ...
      try {
        ...
        if (itemsError || bundlesError) {
          throw itemsError || bundlesError;
        }
    ...
    const totalAmount = items.reduce((total, item) => total + (item.price * item.quantity), 0) +
                       bundles.reduce((total, bundle) => total + (parseFloat(bundle.bundlePrice) * bundle.quantity), 0);
    ...
    console.log('CartProvider: Rendering with value:', { itemCount, totalAmount });
    ...
    console.error('CartProvider: Error initializing:', error);
    return <div>Cart Error: {(error as Error).message}</div>;
  }
};
